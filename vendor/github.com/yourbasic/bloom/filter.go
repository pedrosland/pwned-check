...
func (f *Filter) AddHash(b [20]byte) bool {
	// uint64 has (duh) 64 bits
	// b has 160
	// b = 64 + 32 + 64
...
func (f *Filter) AddHash2(b []byte) bool {
	// uint64 has (duh) 64 bits
	// b has 160
	// b = 64 + 32 + 64
	h1 := binary.BigEndian.Uint64(b[0:8]) + uint64(binary.BigEndian.Uint32(b[8:12]))
...
func (f *Filter) TestHash(b [20]byte) bool {
	h1 := binary.BigEndian.Uint64(b[0:8]) + uint64(binary.BigEndian.Uint32(b[8:12]))
	h2 := binary.BigEndian.Uint64(b[12:])
...
func (f *Filter) TestHash2(b []byte) bool {
	h1 := binary.BigEndian.Uint64(b[0:8]) + uint64(binary.BigEndian.Uint32(b[8:12]))
	h2 := binary.BigEndian.Uint64(b[12:])
...
func (f *Filter) EstimateFalsePositiveRateHash(n uint) (fpRate float64) {
	rounds := uint32(10000)
	// f.ClearAll()
	n1 := make([]byte, 4)
	h := sha1.New()
